...
impl GestureDetector {
    pub fn new(threshold: i32) -> Self {
        Self { threshold, state: State::Idle, last_x: None, last_y: None, last_tap: None }
    }

    pub fn last_position(&self) -> (Option<i32>, Option<i32>) {
    ...
    pub fn feed(&mut self, event: TouchEvent) -> Option<Gesture> {
        match event {
            TouchEvent::TrackingStart => {
                if let State::Tracking { .. } = self.state {
                    return None;
                }
...
mod tests {
    ...
    #[test]
    fn test_swipe_up() {
        let mut d = GestureDetector::new(80);
